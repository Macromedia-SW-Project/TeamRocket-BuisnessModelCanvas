...
namespace HyperCasual.Runner
{
    ...
    public class LevelCompleteScreen : View
    {
        ...
        [SerializeField] Sprite filledStar; // Assign the filled star sprite
        [SerializeField] Sprite emptyStar;  // Assign the empty star sprite
        ...
        void DisplayStars(int count)
        {
            ...
            if (m_Stars.Length > 0 && count >= 0 && count <= m_Stars.Length)
            {
                ...
                {
                    m_Stars[i].sprite = i < count ? filledStar : emptyStar;
                    m_Stars[i].color = new Color(m_Stars[i].color.r, m_Stars[i].color.g, m_Stars[i].color.b, 1f); // Ensure they are visible
                }
            }
        }
    }
